...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$EditOneDiversity)
      rows.sd <- sd(rows$EditOneDiversity)
...
ggsave("EditOneDiversity-curve-noStd.pdf", width = 9, height = 3)
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    finalTestFit.df <- rbind(finalTestFit.df, data.frame(Scenario = rep(scenario.name, nrow(rows)),
                                                         ...
                                                         Run = rows$Run,
                                                         EditOneDiversity = rows$EditOneDiversity))
  }
}
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$EditOneDiversity), mean(rows1$EditOneDiversity), sd(rows1$EditOneDiversity),
              min(rows2$EditOneDiversity), mean(rows2$EditOneDiversity), sd(rows2$EditOneDiversity),
              min(rows3$EditOneDiversity), mean(rows3$EditOneDiversity), sd(rows3$EditOneDiversity)))
}
