...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$PhenotypeDiversity)
      rows.sd <- sd(rows$PhenotypeDiversity)
...
g <- g + labs(y = "Phenotype Diversity")
...
ggsave("PhenotypeDiversity-curve-noStd.pdf", width = 10, height = 5)
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    finalTestFit.df <- rbind(finalTestFit.df, data.frame(Scenario = rep(scenario.name, nrow(rows)),
                                                         ...
                                                         Run = rows$Run,
                                                         PhenotypeDiversity = rows$PhenotypeDiversity))
  }
}
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$PhenotypeDiversity), mean(rows1$PhenotypeDiversity), sd(rows1$PhenotypeDiversity),
              min(rows2$PhenotypeDiversity), mean(rows2$PhenotypeDiversity), sd(rows2$PhenotypeDiversity),
              min(rows3$PhenotypeDiversity), mean(rows3$PhenotypeDiversity), sd(rows3$PhenotypeDiversity)))
}
