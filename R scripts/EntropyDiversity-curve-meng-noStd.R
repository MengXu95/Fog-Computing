...
working_dir <- "D:/xumeng/PhdMainCode/Paper 5 - Cluster/new grid kmeans/"
setwd(working_dir)

sprintf("------------------------Start------------------------------")
algos <- c("pBaselineMTGPD", "pClusterMTGPD", "psameClusterMTGPD")
algo.names <- c("pBaselineMTGPD", "pClusterMTGPD", "psameClusterMTGPD")

objectives <- rep(c("max-flowtime", "mean-flowtime"))
utils <- c(rep(0.85, 2))
ddfactors <- rep(1.5, 2)

scenarios.name <- c("<Fmax, 0.85>", "<Fmean, 0.85>")
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$EntropyDiversity)
      rows.sd <- sd(rows$EntropyDiversity)
...
ggsave("EntropyDiversity-curve-noStd.pdf", width = 10, height = 5)
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    finalTestFit.df <- rbind(finalTestFit.df, data.frame(Scenario = rep(scenario.name, nrow(rows)),
                                                         ...
                                                         Run = rows$Run,
                                                         EntropyDiversity = rows$EntropyDiversity))
  }
}
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$EntropyDiversity), mean(rows1$EntropyDiversity), sd(rows1$EntropyDiversity),
              min(rows2$EntropyDiversity), mean(rows2$EntropyDiversity), sd(rows2$EntropyDiversity),
              min(rows3$EntropyDiversity), mean(rows3$EntropyDiversity), sd(rows3$EntropyDiversity)))
}
