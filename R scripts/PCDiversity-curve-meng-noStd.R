...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$PCDiversity)
      rows.sd <- sd(rows$PCDiversity)
...
ggsave("PCDiversity-curve-noStd.pdf", width = 10, height = 5)
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$PCDiversity), mean(rows1$PCDiversity), sd(rows1$PCDiversity),
              min(rows2$PCDiversity), mean(rows2$PCDiversity), sd(rows2$PCDiversity),
              min(rows3$PCDiversity), mean(rows3$PCDiversity), sd(rows3$PCDiversity)))
}
