...
#algos <- c("jMTAGP","mMTAGP")
#algo.names <- c("jMTAGP-0-0","mMTAGP-512-0.015-no surrogate")
...
for (s in 1:length(scenarios.name)) {
  ...
  best_makespans_a = as.numeric(subset(dfa, Generation == 50)$TrainTime)
  ...
  best_makespans_b = as.numeric(subset(dfb, Generation == 50)$TrainTime)
