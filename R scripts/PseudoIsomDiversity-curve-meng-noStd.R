...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$PseudoIsomiDIversity)
      rows.sd <- sd(rows$PseudoIsomiDIversity)
...
g <- g + labs(y = "PseudoIsomorphs Diversity")
...
ggsave("PseudoIsomorphsDiversity-curve-noStd.pdf", width = 10, height = 5)
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    finalTestFit.df <- rbind(finalTestFit.df, data.frame(Scenario = rep(scenario.name, nrow(rows)),
                                                         ...
                                                         Run = rows$Run,
                                                         PseudoIsomiDIversity = rows$PseudoIsomiDIversity))
  }
}
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$PseudoIsomiDIversity), mean(rows1$PseudoIsomiDIversity), sd(rows1$PseudoIsomiDIversity),
              min(rows2$PseudoIsomiDIversity), mean(rows2$PseudoIsomiDIversity), sd(rows2$PseudoIsomiDIversity),
              min(rows3$PseudoIsomiDIversity), mean(rows3$PseudoIsomiDIversity), sd(rows3$PseudoIsomiDIversity)))
}
