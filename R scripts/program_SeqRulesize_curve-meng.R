...
g <- g + labs(y = "sequencing rule size")
...
ggsave("sequencingSize-curve.pdf", width = 9, height = 3.5)
...
for (s in 1:length(algos)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              algos[s],
              min(rows1$SeqRuleSize), mean(rows1$SeqRuleSize), sd(rows1$SeqRuleSize),
              min(rows2$SeqRuleSize), mean(rows2$SeqRuleSize), sd(rows2$SeqRuleSize),
              min(rows3$SeqRuleSize), mean(rows3$SeqRuleSize), sd(rows3$SeqRuleSize),
              min(rows4$SeqRuleSize), mean(rows4$SeqRuleSize), sd(rows4$SeqRuleSize)))
}
