...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    for (g in 1:generations) {
      ...
      rows.mean <- mean(rows$GenotypeDiversity)
      rows.sd <- sd(rows$GenotypeDiversity)
...
ggsave("GenotypeDiversity-curve-noStd.pdf", width = 10, height = 5)
...
for (s in 1:length(scenarios.name)) {
  ...
  for (a in 1:length(algos)) {
    ...
    finalTestFit.df <- rbind(finalTestFit.df, data.frame(Scenario = rep(scenario.name, nrow(rows)),
                                                         ...
                                                         Run = rows$Run,
                                                         GenotypeDiversity = rows$GenotypeDiversity))
  }
}
...
for (s in 1:length(scenarios.name)) {
  ...
  & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f) & %.2f - %.2f(%.2f)\\\\\n",
              scenarios.name[s],
              min(rows1$GenotypeDiversity), mean(rows1$GenotypeDiversity), sd(rows1$GenotypeDiversity),
              min(rows2$GenotypeDiversity), mean(rows2$GenotypeDiversity), sd(rows2$GenotypeDiversity),
              min(rows3$GenotypeDiversity), mean(rows3$GenotypeDiversity), sd(rows3$GenotypeDiversity)))
}
